...
public class EmailUtils {
	...
	private static MimeMessage createMsg(Session session, String myAccount, User user) {
		...
		try {
			...
			String url = "http://"+ip+":8080/codeactivate?e="+ Base64Utils.encode(user.getEmail())+"&c="+Base64Utils.encode(user.getActivecode());
			//设置正文内容
			message.setContent(user.getUsername()+",您好：<br>欢迎您在我们17锋迷注册会员，期待您的激活:<a href='"+url+"'>"+url+"</a>","text/html;charset=utf-8");
			//设置发送日期
